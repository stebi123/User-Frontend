...
export default function Auditoriums() {
  ...
  const { auditoriumEvents, loading, error } = useVenues();
    ...
    <Animated.View entering={FadeInDown}>
      <Link href={`/venue-details/${item.id}`} asChild>
        <TouchableOpacity style={styles.venueCard}>
          <Image
            source={{ uri: "https://via.placeholder.com/300x150.png?text=Fallback" }}
  ...
  return (
    <View style={styles.container}>
      ...
      {loading ? (
        ...
        <>
          <Animated.View entering={FadeInDown}>
            <Text style={styles.title}>Auditorium Events</Text>
          </Animated.View>
          <FlatList
